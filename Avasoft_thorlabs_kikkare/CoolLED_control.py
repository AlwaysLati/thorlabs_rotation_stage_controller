...
class WorkingThread(QThread):
    ...
    def light(self):
        ...
        for i, task in enumerate(self.seqlist):
            ...
            self.emit(pyqtSignal('update(QString)'), "Working: " + str(i + 1) + '/' + str(len(self.seqlist)) + '   ')
...
class MainWindow(QMainWindow):
    ...
    def setGridLayout(self):
        ...
        instructions = QLabel(
            "LED 1: A \t LED 2: B \t LED 3: C \t LED 4: D \txI###: set intensity for channel x \t W###: wait\nEnter your sequence in the following format: \n\nW20 A2 C2 A2 C2 A2 C2 A5 W100 etc. (freely)")

        self.useframes = QCheckBox('Use frames instead of seconds')
        self.useframes.setChecked(False)
        self.useframes.stateChanged.connect(self.setframemode)

        frameboxl = QLabel("frame time (s): ")
        self.framebox = QLineEdit()
    ...
    def initport(self):
        ...
        for count, port in enumerate(ports):
            self.statusBar().showMessage("Trying port: " + str(count))
            try:
                ...
                pass
            if count == 9:
                self.statusBar().showMessage("pE not found. Init for retry.")
    ...
    def doplot(self):
        ...
        self.totaltime.setText('{:.2f}'.format(self.totalcount) + 's')
    ...
    def runLEDs(self):
        ...
        self.connect(self.workingthread, pyqtSignal("update(QString)"), self.updatesbar)
    ...
    def openfile(self):
        ...
        if self.path:
            f = QFileDialog.getOpenFileNameAndFilter(directory=self.path, filter="pE Control files (*.pE2)")[0]
        else:
        ...
        self.path = QFileInfo(f).path()  # store path for next time
...
app = QApplication([])
win = MainWindow()  # Create Widget eq. Window
